              ...
              <div class="card-body box-profile">
                ...
                <ul class="list-group list-group-unbordered mb-3">
                  <li class="list-group-item">
                    <b>Followers</b>
                    <a class="float-right">{{ otherUser.followersCount }}</a>
                  </li>
                  <li class="list-group-item">
                    <b>Following</b>
                    <a class="float-right">{{ otherUser.followingCount }}</a>
                  </li>
                </ul>
            ...
            <div class="card">
              <div class="card-header p-2">
                <ul class="nav nav-pills">
                  ...
                  <li
                    v-if="user.username == otherUser.username"
                    class="nav-item"
                  >
                        ...
                        <form @submit.prevent="postTweet(newTweet)">
                          ...
                          <div class="card-body">
                            <div class="form-group">
                              <label>Text</label>
                              <textarea
                                v-model="newTweet.tweetText"
                                class="form-control"
                                rows="3"
                                placeholder="Your thoughts 🤔️ here..."
                                autofocus
                              ></textarea>
                            </div>
                          </div>
...
export default {
  ...
  computed: {
    ...mapGetters({
      tweets: 'store/tweets',
      otherUser: 'store/otherUser',
      user: 'store/user',
      toggleFollow: 'store/toggleFollow',
      isPostTweetBusy: 'store/isPostTweetBusy'
    })
  },
  watch: {
    '$route.path'() {
  ...
  methods: {
    ...mapActions({ postTweet: 'store/postTweet', putUser: 'store/putUser' }),
    onCreated() {
      if (this.$route.path.startsWith('/feeds'))
        this.$store.dispatch('store/getFeeds')
      else this.$store.dispatch('store/getProfile', this.$route.params.username)
    }
  }
}
</script>
